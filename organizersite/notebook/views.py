...
def validate_new_note_view(request):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, 'New notes added')
    return redirect(reverse('notebook:home'))
...
class NoteUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_title'] = f'Edit notes {self.object.title}'
        return context

    def form_valid(self, form):
        form.save()
        messages.success(self.request, f'Notes edited')
...
def pinned_view(request, pk):
    ...
    instance.save()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'), reverse('notebook:home'))
